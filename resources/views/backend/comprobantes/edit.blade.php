...
@section('content')
    ...
    <h3 class="mb-4 text-gray-800">Editar Comprobante</h3>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('comprobantes.update', $comprobante->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="pago" id="pago" class="custom-select">
                            ...
                            <option {{ $comprobante->pago->id == $pago->id ? 'selected' : '' }} value="{{ $pago->id }}">{{ $pago->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="cuenta" id="cuenta" class="custom-select">
                            ...
                            <option {{ $comprobante->cuenta->id == $cuenta->id ? 'selected' : '' }} value="{{ $cuenta->id }}">{{ $cuenta->grupo->nombre .'/'. $cuenta->banco .'/'. $cuenta->numero }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" id="numero" name="numero" class="form-control" value="{{ $comprobante->numero }}">
                    </div>
                </div>
                <div class="form-group row">
                    <label for="fecha" class="col-form-label col-sm-2">Fecha de Pago</label>
                    <div class="col-sm-10">
                        <input type="date" id="fecha" name="fecha" class="form-control" value="{{ $comprobante->fecha_pago }}">
                    </div>
                </div>
                <div class="form-group row">
                    <label for="monto" class="col-form-label col-sm-2">Monto del Comprobante</label>
                    <div class="col-sm-10">
                        <input type="number" id="monto" name="monto" step="0.1" class="form-control" value="{{ $comprobante->monto }}">
                    </div>
                </div>
                <div class="form-group row">
                ...
                <button type="submit" class="btn btn-sm btn-primary">Editar Comprobante</button>
            </form>
        </div>
    </div>
